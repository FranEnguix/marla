...
def _global_progress(total=0, with_root=0):
    return {"visible_sensitive_targets_total": total, "visible_sensitive_targets_with_root": with_root}


def test_shipped_knowledge_base_tells_finish_to_score_high_once_objective_complete():
    kb = load_knowledge_base("package://marla/knowledge/nasimemu_rules.yaml")
    obs = {"local_hosts": [], "global_progress": _global_progress(total=1, with_root=1)}
...
def test_services_unknown_flag():
    obs = {"local_hosts": [_host(reachable=True, known_services=0)]}
    assert "services_unknown" in compute_observation_flags(obs)

    obs2 = {"local_hosts": [_host(reachable=True, known_services=3)]}
    assert "services_unknown" not in compute_observation_flags(obs2)


def test_user_access_present_flag():
    assert "user_access_present" in compute_observation_flags({"local_hosts": [_host(access="user")]})
    assert "user_access_present" in compute_observation_flags({"local_hosts": [_host(access="root")]})
    assert "user_access_present" not in compute_observation_flags({"local_hosts": [_host(access="none")]})


def test_root_access_missing_flag():
    assert "root_access_missing" in compute_observation_flags({"local_hosts": [_host(access="user")]})
    assert "root_access_missing" not in compute_observation_flags({"local_hosts": [_host(access="root")]})


def test_empty_observation_has_no_positive_flags_but_root_missing_is_vacuously_true():
    flags = compute_observation_flags({"local_hosts": []})
    assert flags == {"root_access_missing"}


def test_all_sensitive_hosts_captured_flag():
    obs = {"local_hosts": [], "global_progress": _global_progress(total=2, with_root=2)}
    assert "all_sensitive_hosts_captured" in compute_observation_flags(obs)

    obs_incomplete = {"local_hosts": [], "global_progress": _global_progress(total=2, with_root=1)}
    assert "all_sensitive_hosts_captured" not in compute_observation_flags(obs_incomplete)

    obs_none_sensitive = {"local_hosts": [], "global_progress": _global_progress(total=0, with_root=0)}
    assert "all_sensitive_hosts_captured" not in compute_observation_flags(obs_none_sensitive)


def test_all_sensitive_hosts_captured_flag_is_global_even_when_local_hosts_is_empty():
    """The capture-target flag is GLOBAL/whole-network (matching FINISH's
    own global scope), not derived from local_hosts -- a subnet with no
    sensitive host of its own can still see this flag when the completed
    sensitive target lives in a DIFFERENT (unconsulted) subnet."""
    obs = {"local_hosts": [_host(access="none")], "global_progress": _global_progress(total=1, with_root=1)}
    assert "all_sensitive_hosts_captured" in compute_observation_flags(obs)
...
def test_services_unknown_flag_treats_empty_list_the_same_as_zero_count():
    ...
    obs = {"local_hosts": [{"target": "host-1-0", "access": "none", "reachable": True, "known_services": []}]}
    assert "services_unknown" in compute_observation_flags(obs)
...
def test_retrieve_matches_rule_with_no_triggers_regardless_of_context():
    ...
    matched = retrieve_rules(kb, {"local_hosts": []}, {"finish"})
...
def test_retrieve_requires_all_observation_flags():
    ...
    assert retrieve_rules(kb, {"local_hosts": []}, set()) == []
    # both active -> should match
    assert [r.id for r in retrieve_rules(kb, {"local_hosts": [_host(access="user")]}, set())] == ["needs-both"]
...
def test_retrieve_requires_any_legal_action_type_overlap():
    ...
    assert retrieve_rules(kb, {"local_hosts": []}, {"finish"}) == []
    assert [r.id for r in retrieve_rules(kb, {"local_hosts": []}, {"exploit"})] == ["scan-rule"]
